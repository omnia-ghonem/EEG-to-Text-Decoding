...
class BrainTranslator(nn.Module):
    def __init__(self, gpt2, in_feature = 840, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048):
    ...
    def forward(self, input_embeddings_batch, input_masks_batch, input_masks_invert, target_ids_batch_converted, lenghts_words, word_contents_batch, word_contents_attn_batch, stepone, subject_batch, device, features=False):
        ...
        if stepone==True:
            words_embedding = self.gpt2.transformer.wte(word_contents_batch)
            loss = nn.MSELoss()
